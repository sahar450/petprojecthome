    ...
    <Box sx={{ width: "100%", backgroundColor: "transparent" }}>
      ...
      {isDesktop && (
        <TableContainer component={Paper}>
          <Table stickyHeader sx={{ direction: "rtl", backgroundColor: " transparent" , boxShadow:"none"}}>
            <TableHead>
              <TableRow>
                {cols.map((c, i) => (
                  <ThheadCell
            ...
            <TableBody>
              {paginatedData.map((row, index) => (
                <TableRow key={row.id ?? Math.random()} sx={{ backgroundColor: "transparent" }}>
                  <TableCell colSpan={cols.length + (action ? 1 : 0)} sx={{ p: 0, border: "none" }}>
                    <Paper
                      elevation={0}
                      sx={{
                        p: 1,
                    ...
                    >
                      ...
                      {action && (
                        <Box sx={{ px: 2, py: 1, display: "flex", gap: 1 }}>
                          <IconButton onClick={() => openModal("view", row)}>{eyesmenu}</IconButton>
                          <IconButton onClick={() => openModal("edit", row)}>{edit}</IconButton>
                          <IconButton onClick={() => openModal("delete", row)}>{trash}</IconButton>
                        </Box>
                      )}
                    </Paper>
                  </TableCell>
