...
export default function StickyHeadTable({
  ...
  return (
    <Box sx={{ width: "100%", backgroundColor: "transparent" }}>
      <Box sx={{ display: param ? "none" : { xs: "none", md: "block" } }}>
        <TableContainer component={Paper}>
          <Table stickyHeader sx={{ direction: "rtl", backgroundColor: "#FBFBFB" }}>
            <TableHead>
              <TableRow>
                {cols.map((c, i) => (
                  <ThheadCell key={i}>{c.label}</ThheadCell>
                ))}
            ...
            <TableBody>
              {tableData.map((row) => (
                <TableRow key={row.id}>
                  <ThCell colSpan={cols.length + (action ? 1 : 0)} sx={{ p: 0, border: "none",direction: "rtl" }}>
                    <Box
                      sx={{
                        display: "flex",
                        borderRadius: "12px",
                        overflow: "hidden",
                        backgroundColor: row.id % 2 === 0 ? "#EDF1F2" : "#FFFFFF",
                      }}
                    >
                      {cols.map((col, ci) => (
                        <Box key={`${row.id}-${col.key}`} sx={{ flex: 1, p: 1.5, textAlign: "right" }}>
          ...
          >
            ...
            {cols.map((col, ci) => (
              ...
              >
                ...
                <Txt mycolor="#333" mysize="13px" myweight="400" myfont="Yekan">
                  {renderCellValue(row, col)}
                </Txt>
              </Box>
            ))}
